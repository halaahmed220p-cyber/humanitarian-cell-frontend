    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-[9999] bg-white shadow-md border-b border-slate-200" dir="rtl">
        <div className="max-w-[1400px] mx-auto px-6 py-4 flex justify-between items-center">
          ...
          {/* الشعار يمين */}
          {program ? (
            <button onClick={() => navigate('/programs')} className="flex items-center gap-2 text-slate-700 hover:text-slate-900 px-4 py-2 rounded-xl hover:bg-slate-100 cursor-pointer">
          ...
          <nav className="hidden md:flex items-center gap-6">
            <NavLink to="/" className="text-slate-700 hover:text-[#c9a84c] font-medium">{t('home') || "الرئيسية"}</NavLink>
            <HashLink smooth to="/#about" className="text-slate-700 hover:text-[#c9a84c] font-medium">{t('about') || "من نحن"}</HashLink>
            <NavLink to="/projects" className="text-slate-700 hover:text-[#c9a84c] font-medium">{t('projects') || "المشاريع"}</NavLink>
            <NavLink to="/programs" className="text-slate-700 hover:text-[#c9a84c] font-medium">{t('programs') || "البرامج"}</NavLink>
            <NavLink to="/news" className="text-slate-700 hover:text-[#c9a84c] font-medium">{t('newsAndReports') || "الأخبار والتقارير"}</NavLink>
          </nav>

          {/* الأزرار يسار */}
          <div className="flex items-center gap-3">
            <button onClick={toggleLanguage} className="px-3 py-2 rounded-xl border border-slate-200 bg-slate-50 text-slate-700 hover:bg-slate-100 transition text-sm font-medium flex items-center gap-1.5 cursor-pointer shadow-sm">
